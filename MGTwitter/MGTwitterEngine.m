    ...
    switch (responseType) {
        ...
        case MGTwitterUser:
			if ([self _isValidDelegateForSelector:@selector(userInfoReceived:forRequest:)])
            {
                ...
                NSString* next_cursor = nil;
                NSString* previous_cursor = nil;
                NSString* tmp_next = nil;
                NSString* tmp_prev = nil;
                
                // find the next_cursor and previous_cursor entries, strip them out of
                // the separate dictionary entries
                // and add them to the individual dictionaries
                
                for (NSMutableDictionary* objDict in parsedObjects){
                    tmp_next = [objDict valueForKey:@"next_cursor"];
                    tmp_prev = [objDict valueForKey:@"previous_cursor"];
                    if (tmp_next){
                        next_cursor = tmp_next;
                        DLog(@"WJL --> found next_cursor: %@",next_cursor);
                    }
                    if (tmp_prev){
                        previous_cursor = tmp_prev;
                    }
                    if (tmp_next == nil && tmp_prev == nil){
                        [newObjs addObject:objDict];
                    }
                }
                for (NSMutableDictionary* objDict in newObjs){
                    [objDict setValue:next_cursor forKey:@"next_cursor"];
                    [objDict setValue:previous_cursor forKey:@"previous_cursor"];
                }
                
				[_delegate userInfoReceived:newObjs forRequest:identifier];
            }
            break;
        case MGTwitterDirectMessages:
        case MGTwitterDirectMessage:
...
{
    return [self getFollowersIncludingCurrentStatus:flag forScreenName:nil withCursor:nil];
}

- (NSString *)getFollowersIncludingCurrentStatus:(BOOL)flag withCursor:(NSString*)cursor
{
    return [self getFollowersIncludingCurrentStatus:flag forScreenName:nil withCursor:cursor];
}

- (NSString *)getFollowersIncludingCurrentStatus:(BOOL)flag forScreenName:(NSString*) screenName
{
    return [self getFollowersIncludingCurrentStatus:flag forScreenName:screenName withCursor:nil];
}

- (NSString *)getFollowersIncludingCurrentStatus:(BOOL)flag forScreenName:(NSString*) screenName withCursor:(NSString*) cursor
{
